print('ДОМАШНЯЯ РАБОТА К УРОКУ 3')

print('ЗАДАНИЕ 1. Методами строк очистить текст от знаков препинания')
...
punctuation = ''
print(str.replace(',', punctuation).replace('.', punctuation).replace('—', punctuation).replace('?', punctuation).replace('!', punctuation).replace('«', punctuation).replace('»', punctuation).replace(';', punctuation).replace('(', punctuation).replace(')', punctuation))

print('ЗАДАНИЕ 2. Сформировать list со словами (split)')

x = str.split()
print(x)

print('ЗАДАНИЕ 3. привести все слова к нижнему регистру (map)')

new_x = list(map(lambda a:a.lower(),x))
print(new_x)

print('ЗАДАНИЕ 4. получить из list пункта 3 dict, ключами которого являются слова, а значениями их количество появлений в тексте')
new_xx = {}
dict = new_x*1
for element in dict:
    b = 0
...
print('Вывести 5 наиболее часто встречающихся слов (sort), вывести количество разных слов в тексте (set).')

new_xx = list(new_xx.items())
new_xx.sort(key=lambda i: i[1])
new_xx.reverse()
count=0
words5=''
for i in new_xx:
...
print('5 наиболее часто встречающихся слов:', words5 )


set_text_1 = set(new_xx)
set_text_2 = set(new_xx)
set_text_3 = set_text_1.intersection(set_text_2)
count = 0
for element in set_text_3:
    count+=1
